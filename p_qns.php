...
if (!isset($postData['p_id']) || !isset($postData['responses']) || !is_array($postData['responses'])) {
    echo "Invalid data: p_id or responses missing or not an array";
    exit;
}
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO $tableName (questions, type, response, response_time) VALUES (:questions, :type, :response, :response_time)");
    ...
    if (!empty($dangerSymptoms)) {
        ...
        $nameStmt = $conn->prepare("SELECT name FROM p_login WHERE p_id = :p_id");
