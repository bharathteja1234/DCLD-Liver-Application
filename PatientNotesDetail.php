...
if ($method === 'GET') {
    if (isset($_GET['p_id']) && isset($_GET['Notes_Index']) && isset($_GET['notes_date'])) {
        ...
        $sql = "SELECT Notes FROM " . $tableName . " WHERE Notes_Index = :Notes_Index AND notes_date = :notes_date";
    ...
    if ($p_id && $Notes && $notes_date && $Notes_Index) {
        $tableName = "patient_" . $p_id;
        $sql = "UPDATE " . $tableName . " SET Notes = :Notes WHERE Notes_Index = :Notes_Index AND notes_date = :notes_date";
