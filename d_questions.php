...
function addQuestion($question, $type) {
    global $conn;
    try {
        $stmt = $conn->prepare("INSERT INTO questions (question, type) VALUES (:question, :type)");
...
function fetchQuestions() {
    ...
    $questions = array();
    try {
        $stmt = $conn->query("SELECT id, question, type FROM questions");
...
function updateQuestion($id, $question, $type) {
    global $conn;
    try {
        $stmt = $conn->prepare("UPDATE questions SET question=:question, type=:type WHERE id=:id");
        $stmt->bindParam(':question', $question);
        $stmt->bindParam(':type', $type);
