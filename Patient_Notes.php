...
if ($method === 'GET') {
    if (isset($_GET['p_id'])) {
        ...
        $sql = "SELECT Notes_Index, notes_date FROM $tableName WHERE Notes_Index IS NOT NULL AND notes_date IS NOT NULL";
    ...
    if ($p_id && $Notes_Index && $notes_date) {
        $tableName = "patient_" . $p_id;
        $sql = "INSERT INTO $tableName (Notes_Index, notes_date) VALUES (:Notes_Index, :notes_date)";
        ...
        if ($stmt->execute()) {
            echo json_encode(array("Notes_Index" => $Notes_Index, "notes_date" => $notes_date));
        } else {
            http_response_code(500);
    ...
    if ($p_id && $Notes_Index) {
        $tableName = "patient_" . $p_id;
        $sql = "DELETE FROM $tableName WHERE Notes_Index = :Notes_Index";
